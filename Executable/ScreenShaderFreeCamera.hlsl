...
Texture2D           gtxtAmbient		 : register(t3);
Texture2D           gtxtSpecualar	 : register(t4);
...
float4 PS(VS_TEXTURED_OUTPUT input) : SV_Target{
	...
	float4 ambient = gtxtAmbient.Sample(gssSamplerState, input.texCoord);
	float4 specular = gtxtSpecualar.Sample(gssSamplerState, input.texCoord);
	...
	*/
	...
	dMaterial.m_cAmbient = ambient;
	dMaterial.m_cSpecular = specular;
	//조명 계산에 필요한 구조체 정의
	...
	if (normal.w == 1)
	{
		if(fog)cColor = Fog(cColor, positionW.xyz);
	}
	else if (normal.w == 2)
	{
		if (fog)cColor = FogAlpahRemove(cColor, positionW.xyz);
	}
	else if (normal.w == 4)
	{
		if (fog)cColor = FogDensity(cColor, 2.0f, positionW.xyz);
	}
	else if (normal.w == 3)
	{
		if (fog)cColor = FogSpace(cColor, 0.9);
	}
	else if (normal.w == 5)
	{
		if (fog) cColor = FogWaterFall(cColor, 0.7f, positionW.xyz);
	}
