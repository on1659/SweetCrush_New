...
bool CGameFramework::OnCreate(HINSTANCE hInstance, HWND hMainWnd)
{
	...
	if (m_pd3dLimBuffer)m_pd3dLimBuffer->Release();
...
bool CGameFramework::CreateDirect3DDisplay()
{
	...
	HRESULT hResult = S_OK;
		...
		for (UINT i = 0; i < nDriverTypes; i++)
		{
			...
			if (SUCCEEDED(hResult = D3D11CreateDeviceAndSwapChain(nullptr, nd3dDriverType, nullptr, dwCreateDeviceFlags, pd3dFeatureLevels, nFeatureLevels, D3D11_SDK_VERSION, &dxgiSwapChainDesc, &m_pDXGISwapChain, &m_pd3dDevice, &nd3dFeatureLevel, &m_pd3dDeviceContext))) break;
		}
	#else
...
void CGameFramework::SaveTwBarData(const std::string& path)
{
	...
	for (int i = 0; i < 4; ++i) ofs << "outline_color " << m_stOutLine->color[i] << std::endl;
...
#ifdef _MULIT_RENDER_TARGET_
	//Texture2D
	if (m_pd3dtxtPosition) m_pd3dtxtPosition->Release();
	m_pd3dtxtPosition = nullptr;

	if (m_pd3dtxtNormal)m_pd3dtxtNormal->Release();
	m_pd3dtxtNormal = nullptr;

	if (m_pd3dtxtDiffuse)m_pd3dtxtDiffuse->Release();
	m_pd3dtxtDiffuse = nullptr;

	//Shader Resource View
	if (m_pd3dsrvPosition)m_pd3dsrvPosition->Release();
	m_pd3dsrvPosition = nullptr;

	if (m_pd3dsrvNormal)m_pd3dsrvNormal->Release();
	m_pd3dsrvNormal = nullptr;

	if (m_pd3dsrvDiffuse)m_pd3dsrvDiffuse->Release();
	m_pd3dsrvDiffuse = nullptr;
	//Render Target View
	if (m_pd3drtvPosition) m_pd3drtvPosition->Release();
	m_pd3drtvPosition = nullptr;

	if (m_pd3drtvNormal)m_pd3drtvNormal->Release();
	m_pd3drtvNormal = nullptr;

	if (m_pd3drtvDiffuse)m_pd3drtvDiffuse->Release();
	m_pd3drtvDiffuse = nullptr;
...
void CGameFramework::FrameAdvance()
{
	...
	#pragma region[Font]
		POINT pt = INPUT->GetMousePoint();
		FONTMGR->MessagePush("카툰", pt.x, pt.y, 15, YT_Color(0, 0, 0), PIVOT_HORIZONTAL_CENTER | PIVOT_VERTICLE_CENTER, L"(%dㆍ%d)", pt.x, pt.y);
	#pragma endregion
